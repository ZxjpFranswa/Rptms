<template>
  <div class="p-4 sm:p-6 space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <StatCard title="Under Verification" :value="verificationCount" icon="clock" color="blue" />
      <StatCard title="Returned Applications" :value="returnedCount" icon="alert" color="yellow" />
      <StatCard title="Approved Applications" :value="approvedCount" icon="check" color="green" />
...
const verificationCount = computed(
  () =>
    propertyStore.registrations.filter(
      (r) => r.status === 'Under_Verification' || r.status === 'Under_Review',
    ).length,
)
...
const approvedCount = computed(
  () => propertyStore.registrations.filter((r) => r.status === 'Approved').length,
)
