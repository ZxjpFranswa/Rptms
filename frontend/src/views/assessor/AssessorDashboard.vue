<template>
  <div class="p-6 space-y-6">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
      ...
      <StatCard title="Activated Today" :value="activatedTodayCount" icon="check" color="green" />
...
const activatedTodayCount = computed(() =>
  propertyStore.registrations.filter(
    (r) => r.status === 'Active' && r.updatedAt.split('T')[0] === today(),
  ).length,
)
