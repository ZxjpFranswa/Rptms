            ...
            <template v-else>
              ...
              >
                <label class="flex items-center gap-3">
                  ...
                  />
                  <span class="text-sm text-green-800">I confirm this property should be activated for taxation</span>
                </label>
                <button
                  type="button"
                  class="w-full px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed transition"
                  :disabled="!decision.activateNow"
  ...
  try {
    if (decision.value.action === 'approve') {
      await propertyStore.assessorApprove(
        id,
        decision.value.remarks || 'Application approved and activated',
      )
