...
export const usePropertyStore = defineStore('property', () => {
  ...
  const assessorApprove = async (id: string, remarks: string, activateNow = false) => {
    const reg = await approveApplication(id, remarks, activateNow)
    upsertLocal(reg)
    return reg
  }
  ...
  const activateProperty = async (id: string) => {
    const reg = await activateApplication(id)
    upsertLocal(reg)
    return reg
  }
