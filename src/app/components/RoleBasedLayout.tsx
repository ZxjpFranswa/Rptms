...
import { LogOut, Home, CreditCard, FileText, Menu, X, CheckSquare, DollarSign, ScrollText, FileEdit, Bell } from "lucide-react";
  ...
  const getNavItems = () => {
    switch (userRole) {
      ...
      case "clerk":
        return [
          { id: "dashboard", label: "Dashboard", icon: Home },
          { id: "soa", label: "Generate SOA", icon: FileEdit },
          { id: "approvals-status", label: "Approval Status", icon: Bell },
          { id: "history", label: "Payment History", icon: FileText },
        ];
      case "cashier":
        return [
          { id: "dashboard", label: "Dashboard", icon: Home },
          { id: "payment", label: "Process Payment", icon: CreditCard },
          { id: "history", label: "Payment History", icon: FileText },
        ];
      case "taxpayer":
        return [
          { id: "portal", label: "My Account", icon: Home },
        ];
      default:
        return [];
    }
  ...
  return (
    <div className="flex h-screen bg-[#f5f7fa] overflow-hidden">
      ...
      <aside className="hidden lg:flex lg:flex-col bg-[#14522d] w-[321px] flex-shrink-0">
        ...
        {/* User Info */}
        <div className="px-[27px] py-4 border-b border-[rgba(255,255,255,0.13)]">
          <p className="font-['Poppins'] text-white text-[14px] font-semibold">
            {userName}
          </p>
          <p className="font-['Poppins'] text-white/70 text-[12px]">
            {getRoleLabel()}
          </p>
        </div>
        ...
        <div className="px-[27px] pb-[67px]">
          <button
            onClick={onLogout}
            className="w-[136px] h-[48px] bg-white border border-[#059467] rounded-[8px] flex items-center justify-center gap-2 hover:bg-[#059467]/10 transition-colors"
          >
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 bottom-0 bg-[#14522d] w-[280px] flex flex-col">
            ...
            {/* Mobile User Info */}
            <div className="px-4 py-3 border-b border-[rgba(255,255,255,0.13)]">
              <p className="font-['Poppins'] text-white text-[14px] font-semibold">
                {userName}
              </p>
              <p className="font-['Poppins'] text-white/70 text-[12px]">
            ...
            <div className="px-4 pb-8">
              <button
                onClick={onLogout}
                className="w-full h-[48px] bg-white border border-[#059467] rounded-[8px] flex items-center justify-center gap-2"
              >
                <LogOut className="w-[17px] h-[17px] text-[#059467]" />
